# INTENGRANTES: Nicolas Nuñez, Narella Rochetti, Elias Palamidessi
...
def altaProductos():
    ...
    if x == -1:
        ...
        grabarProd(cod, nom, os.path.getsize(afProductos), True)
        print('El producto fue cargado exitosamente...')
    
    elif x != -1:
        alProductos.seek(x)
        rProductos = pickle.load(alProductos)
        if rProductos.disponibilidad == False:
            rProductos.disponibilidad = True
            formatearProductos(rProductos)
            alProductos.seek(x)
            pickle.dump(rProductos, alProductos)
            alProductos.flush()
            print('El producto fue dado de alta exitosamente uwu...')
    else:
...
def altaSilos():
    ...
    if buscarProducto(codP) != -1:
        ...
        if buscarSilos(codS) == -1:
            ...
            print('Silo cargado exitosamente...')
    else:
...
# GRABAR
def grabarProd(cod, nom, pos, t):
    rProductos.codProducto = cod
    rProductos.nomProducto = nom
    rProductos.disponibilidad = t
    formatearProductos(rProductos)
...
def baja():
    consulta()
    cod = validarNum(1, 9999, "Ingrese el codigo del producto a eliminar (0 para salir): ")

    while cod != 0:
        x = buscarProducto(cod)
        if x == -1:
            print("el código que ingresó no existe")
            cod = int(input("ingrese el codigo del producto a eliminar (0 para salir): "))
        else:
            alProductos.seek(x)
            rProductos = pickle.load(alProductos)
            ...
            alProductos.flush()
            cod = 0
            print('El producto fue dado de baja exitosamente...')
...
def consulta():
    ...
    while alProductos.tell() < tam:
        rProductos = pickle.load(alProductos)
        print(rProductos.codProducto, rProductos.nomProducto, rProductos.disponibilidad)
...
def consultaCupos():
    ...
    while alOperaciones.tell() < tam:
        ...
        print(rOperaciones.patente, rOperaciones.fecha, rOperaciones.codProducto, rOperaciones.estado, rOperaciones.bruto, rOperaciones.tara)
    ...
    while alOperaciones.tell() < tamanio and not encontrado:
        ...
        if rOperaciones.patente.strip() == pat.strip() and rOperaciones.fecha.strip() == f:
            if rOperaciones.estado.strip() == "A":
                ...
                encontrado = True
            elif f != rOperaciones.fecha.strip():
                return 25
    if encontrado:
        return(pos)
    else:
...
def menuRecepcion():
    ...
    while opc != 2:
        ...
        if pos == -2:
            ...
            opc = int(input('La patente que ingreso no existe o no concuerda con la fehca.\n2Desea seguir ingresado?\n1: Si\n2: No\n'))
            while opc != 1 and opc != 2:
                opc = int(input('Opcion invalida, vuelva a intentarlo:\n1: Si\n2: No\n'))
